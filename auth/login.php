...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        $stmt = $koneksi->prepare("SELECT id, nama, email, password, role, departemen FROM users WHERE email = ?");
...
<head>
    ...
    <title>Login - Sistem Laporan Kerusakan</title>
...
<body class="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-slate-400">Sistem Manajemen Laporan Kerusakan</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div class="bg-slate-50 px-8 py-4 border-t border-gray-200">
                <p class="text-center text-sm text-gray-600">
                    Belum punya akun? 
                    <a href="register.php" class="text-blue-600 hover:text-blue-800 font-bold hover:underline transition">
                        Daftar sekarang
                    </a>
                </p>
            </div>
        ...
        <!-- Info Login Admin -->
        <div class="mt-6 bg-slate-800 bg-opacity-50 backdrop-blur-sm rounded-lg p-4 border border-slate-700">
            <p class="text-slate-300 text-sm text-center">
                <i class="fas fa-info-circle mr-2"></i>
                <strong>Demo Admin:</strong> admin@example.com | Pass: admin123
            </p>
        </div>
    </div>
</body>
